...
export default function AddStudent() {
  ...
  // Fetch courses and class schedules
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const classSchedulesResponse = await fetch("/api/class-schedules");
        if (classSchedulesResponse.ok) {
          const classSchedulesData = await classSchedulesResponse.json();
          setClassSchedules(classSchedulesData.data?.classSchedules || []);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Tuition data:", formData.tuition);
      console.log("Payment status:", formData.tuition.paymentStatus);
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          course: "",
          class: "",
          parentInfo: { parentName: "", parentPhone: "", parentEmail: "", relationship: "" },
          tuition: { totalAmount: "", paidAmount: "0", paymentStatus: "Chưa thanh toán" },
          notes: "",
          emergencyContact: { name: "", phone: "", relationship: "" },
          academicInfo: { previousEducation: "", currentSchool: "", grade: "" },
        });
        ...
        throw new Error(errorData.message || "Có lỗi xảy ra khi thêm học viên");
      }
    } catch (error) {
  ...
  return (
    <AdminLayout title="Thêm học viên mới">
              ...
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    Đang chuyển hướng về trang quản lý học viên...
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    ...
                    {classSchedules
                      .filter(schedule => 
                        courses.find(course => course.title === formData.course)?.title === schedule.courseId?.title
                      )
                      .map((schedule) => (
                        <option key={schedule._id} value={schedule.className}>
