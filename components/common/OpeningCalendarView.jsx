      ...
      days.push(
        <div
          ...
          className={`h-16 md:h-24 border border-gray-200 p-1 cursor-pointer transition-all duration-200 ${
            isToday 
              ...
              : hasSchedules 
                ? 'bg-gradient-to-br from-pink-50 to-pink-100 border-pink-300 hover:shadow-lg hover:scale-105' 
                : 'hover:bg-gray-50'
          } ${isSelected ? 'bg-pink-100 border-pink-400 shadow-lg ring-2 ring-pink-300' : ''}`}
        ...
        >
          <div className="flex justify-between items-start mb-1">
            ...
            {hasSchedules && (
              <span className="bg-gradient-to-r from-pink-500 to-pink-600 text-white text-xs rounded-full w-4 h-4 md:w-5 md:h-5 flex items-center justify-center shadow-sm font-bold">
                {daySchedules.length}
              </span>
            )}
          </div>
          ...
          {hasSchedules && (
            <div className="space-y-0.5 md:space-y-1">
              {daySchedules.slice(0, 1).map((schedule, index) => (
                <div
                  key={index}
                  className="text-xs bg-gradient-to-r from-pink-100 to-pink-200 text-pink-800 px-1 py-0.5 rounded-md truncate font-medium shadow-sm"
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="p-3 md:p-4 border-b border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
          <div className="text-center bg-pink-50 rounded-lg p-2 md:p-3">
            <div className="text-lg md:text-2xl font-bold text-pink-600">{stats.totalClasses}</div>
      ...
      {selectedDate && calendarData[selectedDate] && (
        <div className="p-3 md:p-4 border-t border-gray-200 bg-gradient-to-r from-pink-50 to-pink-100">
