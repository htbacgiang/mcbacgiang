...
const OpeningScheduleList = ({ year, month }) => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const response = await fetch(`/api/class-schedules/opening-list?${params}`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2 text-gray-600">
          ...
          <span>
            Hiển thị {schedules.length} trong tổng số {pagination.totalItems} lịch khai giảng
          </span>
        </div>
      </div>
      ...
      {schedules.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            Không tìm thấy lịch khai giảng nào phù hợp với bộ lọc của bạn.
          </p>
        </div>
      )}
...
export default OpeningScheduleList;
